...
When everyone is done, each section will be automatically compiled into one report using a special feature of R Markdown called **child documents** (explained below).
...
In our case, the chunk option looks like this:
...
![](images/git_diagram_fork_clone.png){width="569"}
...
Then render the main ``` daly_report_PARENT``.Rmd ``` which will include the completed child docs. NO NEED TO ADD ANY CODE HERE.
...
CHECK LOCALLY TO MAKE SURE IT LOOKS NICE.
...
To submit, each group member should SUBMIT the link to the group's GitHub repo AS A FILE on the course platform. COPY FROM SLIDE.

This will be the same repo for all group members. But each group member should submit the link individually. From the commit history, we will be able to see who contributed to the repo.
